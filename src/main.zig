...
pub fn main() !void {
    ...
    for (palette) |color| {
        const hex_color = helper.rgb2hex(color.r, color.g, color.b);
        const hex_string = helper.u32_2_hexstr(hex_color);
        try helper.printColorSwatch(stdout, hex_color, &hex_string);
    }
    ...
    while (pixel_it.next()) |pixel_color| {
        ...
        const pixel_hex = helper.rgb2hex(r, g, b);
        const closest_palette_color = helper.findClosestPaletteColor(pixel_hex, palette);
